...
def format_docx(docx_path):
    ...
    for shape in doc.inline_shapes:
        try:
            ...
            # Set width to 3.8 inches (compact representation)
            new_width = Inches(3.8)
...
def main():
    ...
    md_path = os.path.join(workspace_dir, "SahilAgarwal_SmartSolution.md")
    plots_dir = os.path.join(workspace_dir, "data", "plots")
    os.makedirs(plots_dir, exist_ok=True)
    img_dest_path = os.path.join(plots_dir, "data_flow.png")
    ...
    if match:
        ...
        if img_data:
            ...
            rel_img_path = "data/plots/data_flow.png"
            processed_content = mermaid_pattern.sub(f"![Data Flow Diagram]({rel_img_path})", content)
        else:
    ...
    temp_md_path = os.path.join(workspace_dir, "SahilAgarwal_SmartSolution_temp.md")
    with open(temp_md_path, 'w', encoding='utf-8') as f:
        f.write(processed_content)
    
    # Run pandoc to convert MD to DOCX
    docx_path = os.path.join(workspace_dir, "SahilAgarwal_SmartSolution.docx")
    ...
    pdf_path = os.path.join(workspace_dir, "SahilAgarwal_SmartSolution.pdf")
