...
def format_docx(docx_path):
    ...
    # 3. Table styling (Ensure table width fits the page and looks neat)
    for table in doc.tables:
        table.autofit = True
        # Set table padding and border settings if needed
        for row in table.rows:
            for cell in row.cells:
                cell.width = Inches(1.5) # Approximate auto width
...
def main():
    ...
    md_path = os.path.join(workspace_dir, "SahilAgarwal_ESGReport.md")
    ...
    temp_md_path = os.path.join(workspace_dir, "SahilAgarwal_ESGReport_temp.md")
    with open(temp_md_path, 'w', encoding='utf-8') as f:
        f.write(content)
    
    # Run pandoc to convert MD to DOCX
    docx_path = os.path.join(workspace_dir, "SahilAgarwal_ESGReport.docx")
    ...
    pdf_path = os.path.join(workspace_dir, "SahilAgarwal_ESGReport.pdf")
